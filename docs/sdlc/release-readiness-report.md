...
- `npm run model:movenet:readiness`: passed and wrote `docs/sdlc/movenet-readiness-report.json` with status `ready`,
  CPU backend, 3904ms load time, 330ms average inference, and 333ms max inference in the latest run.
...
- `npm run feature:doctor`: passed as a command and wrote `docs/sdlc/feature-completion-report.json` with status
  `external-blocked`, 176/179 tasks done, 130/132 backlog items done, 163/163 traceability rows covered, 0 internal gaps,
...
- `npm run release:freshness:doctor`: passed as a command and wrote `docs/sdlc/release-freshness-report.json` with
  status `ready`, 25/25 fresh artifacts, and 0 stale artifacts.
- `npm run release:check`: passed and wrote `docs/sdlc/release-gate-report.json` with 30/30 release steps passing.

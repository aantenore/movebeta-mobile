...
- `npm test`: passed, 112 test files and 456 tests.
...
- `npm run model:movenet:readiness`: passed and wrote `docs/sdlc/movenet-readiness-report.json` with status `ready`,
  CPU backend, 5847ms load time, 321ms average inference, and 331ms max inference in the latest run.
...
- `npm run feature:doctor`: passed as a command and wrote `docs/sdlc/feature-completion-report.json` with status
  `external-blocked`, 151/154 tasks done, 105/107 backlog items done, 138/138 traceability rows covered, 0 internal gaps,

...
| Real cue-validation dataset | product | store | `docs/validation/cue-validation-dataset.json` | `relative-path`, `report-id`, `issue-url` | `npm run validation:cue:starter`<br>`npm run validation:cue:composition`<br>`npm run validation:cue:composition -- --write-dataset`<br>`npm run validation:cue`<br>`npm run validation:cue:doctor` |
| Real cue-validation dataset | product | store | `docs/sdlc/cue-validation-dataset-report.json` | `relative-path`, `report-id`, `issue-url` | `npm run validation:cue:starter`<br>`npm run validation:cue:composition`<br>`npm run validation:cue:composition -- --write-dataset`<br>`npm run validation:cue`<br>`npm run validation:cue:doctor` |

...
- Show a share-safe cue-validation collection runbook in Sessions that combines the current study seed, reviewer
  assignments, worksheet preflight status, current phase, phase owners, next action, and privacy checks without exporting
  coach identities, reviewer scores, raw worksheet contents, report IDs, raw video, local paths, credentials, or tokens.
...
- Provide an on-device live recording guide in the Coach recorder that uses capture calibration, coach lens, elapsed
  time, minimum analysis duration, and recording limit to show local filming prompts without cloud analysis.
...
- Generate a share-safe store release account runbook that orders store metadata, EAS project binding, Expo token, App
  Store Connect, Google Play, native QA evidence, strict EAS gate, and final EAS submit phases without exposing secret
  values, project id values, local paths, raw artifacts, raw video, or token-like values.
...
- Live recording guidance must be generated by a pure configurable module, remain independent from camera APIs, and expose
  deterministic prompts for setup, opening, middle, finish, and near-limit recording phases.
...
- Store release account runbooks must be schema-versioned, generated from store metadata and credential readiness,
  expose only ordered phases, commands, key names, and evidence names, and reject credential values, project id values,
  local paths, raw artifacts, raw video, and token-like strings.
...
- Model delivery lifecycle evidence must distinguish the verified app delivery path from the current browser/device
  model cache state, so release packets can prove PWA model delivery without claiming every installed client has already
  warmed its local cache.

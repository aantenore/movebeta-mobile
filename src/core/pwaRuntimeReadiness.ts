  ...
  const checks = [
    ...
    check({
      action: nativeRuntime
        ...
        : nextOfflineReady
          ? 'Keep manifest, service worker, and cache assets in the exported dist.'
          : 'Verify Cache API and service worker registration before relying on offline launch.',
      detail: nativeRuntime
        ? 'Offline support is handled by native bundles and local storage.'
        : nextOfflineReady
          ? 'Cache API and service worker registration are available for static offline startup.'
          : probe.cacheApiSupported

...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        expect(page.get_by_text("Device readiness", exact=True)).to_be_visible()
        ...
        expect(page.get_by_text("Analysis run load", exact=True)).to_be_visible()
        ...
        expect(page.get_by_text("Runtime surface", exact=True)).to_be_visible()
        expect(page.get_by_text("Offline video", exact=True)).to_be_visible()
        expect(page.get_by_label("Prepare PWA field readiness packet")).to_be_visible()
        page.get_by_label("Prepare PWA field readiness packet").click()
        expect(page.get_by_text("Prepared PWA field readiness")).to_be_visible()
        expect(page.get_by_text('"schemaVersion": "movebeta.pwa-field-readiness.v1"')).to_be_visible()
        expect(page.get_by_text('"rawVideoIncluded": false').last).to_be_visible()
